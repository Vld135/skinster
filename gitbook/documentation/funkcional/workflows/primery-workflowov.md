...
## Пример 4: Массовая продажа инвентаря

Выставляет на продажу весь инвентарь CS2 на множестве аккаунтов за один прогон. Цена для каждого предмета подбирается оптимально, автоподтверждение лотов через Steam Guard включено.

{% file src="../../.gitbook/assets/batch-sell.json" %}
batch-sell.json — готовый workflow для импорта
{% endfile %}

{% hint style="info" %}
После импорта подправьте источник аккаунтов и при необходимости — фильтры продажи (см. раздел "Что настроить под себя").
{% endhint %}

### Цепочка нод

```
Ручной запуск → Account Context → Sell Inventory
                                    ├─ success → Отчёт
                                    └─ failure → Telegram уведомление
```
...
**Account Context:**
* Источник: `Все аккаунты` (или выберите папку / вручную)
...
**Telegram уведомление (на failure):**
* Тип: `Своё сообщение`
* Сообщение: `Произошла ошибка во время выставления на продажу`
...
| Нода | Что изменить |
|------|---------------|
| **Account Context** | Выбрать аккаунты (по умолчанию — все) |
| **Sell Inventory** | Опционально: задать фильтры по типам или стоимости, если не хотите продавать всё подряд |

{% hint style="warning" %}
Автоподтверждение лотов работает только с MaFiles. Если у аккаунта нет MaFile — выставление произойдёт, но потребуется ручное подтверждение в Steam Guard.
{% endhint %}

---

## Пример 5: Автосортировка инвентаря
