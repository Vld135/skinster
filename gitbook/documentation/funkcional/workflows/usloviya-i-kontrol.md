...
{% hint style="warning" %}
Эта нода находится **в разработке** и пока недоступна в интерфейсе. Она будет добавлена в одном из будущих обновлений.
{% endhint %}

Составные условия с AND/OR логикой для проверки различных параметров аккаунтов.

### Планируемые поля для проверки

| Поле | Тип | Описание |
|------|-----|----------|
| Баланс | число | Баланс Steam аккаунта |
| Стоимость инвентаря | число | Общая стоимость инвентаря |
| Количество предметов | число | Количество предметов в инвентаре |
| Количество кейсов | число | Количество кейсов |
| Количество скинов | число | Количество скинов |
| Количество стикеров | число | Количество стикеров |
| Есть Armory Pass | да/нет | Наличие активного пропуска |
| Звёзды Armory | число | Количество звёзд |
| % звёзд Armory | число | Процент звёзд |
| Prime статус | да/нет | Наличие Prime статуса |
| Уровень профиля | число | Уровень Steam-профиля |
| Опыт | число | Количество опыта |
| Возраст аккаунта (дней) | число | Возраст аккаунта в днях |
