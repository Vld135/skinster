...
**9.** В режиме **Local** под настройками находится сетка кнопок управления. В состоянии ожидания большинство кнопок неактивны и подсвечиваются по мере запуска фарма.
...
**10.** В режиме **Background (Serial)** управление сводится к одной круглой кнопке **Старт**. Панель сама запустит аккаунты по очереди, соберёт лобби, отыграет матчи и (при включённых настройках) соберёт дроп.

<figure><img src="../../.gitbook/assets/autofarm-step-9.png" alt=""><figcaption></figcaption></figure>

Кнопка имеет три состояния: ожидание (**Старт**), запуск (загрузка) и активный фарм. Для запуска должно быть выбрано **0** аккаунтов (тогда панель сама возьмёт первую пачку) или **ровно 4**. Когда фарм активен, рядом появляется кнопка **Open Viewer** — она открывает окно предпросмотра запущенных аккаунтов.
...
**11.** В **ручном** режиме доступны кнопки блокировки и фильтры. В шапке показано, сколько всего серверов и сколько заблокировано.
...
**12.** Переключатель **Ручной ↔ Авто** включает автоматический режим. В нём панель сама подбирает серверы во время фарма. Поля **Перемешивания**, **Текущий** и **Следующий** заполняются, когда идёт активная фарм-сессия; пока фарм не запущен, отображается **Ожидание запуска фарм-сессии...**.
...
**13.** На вкладке **Логгер** в реальном времени отображается лог действий фарма. Чипсы-фильтры сверху (**Все** и по вкладкам) позволяют смотреть события отдельной вкладки. Пока фарм не запущен, панель показывает «Системные события будут отображаться здесь.».
